...
Current development version: **0.9.29 — 2026-09-23**
...
Do the work in this thread. Parallel subagents are not how this repository is worked on: a fan-out spends cold contexts re-deriving what the session already holds, and it has cost this project a rate limit. A review runs its angles one after another here, never at the same time, and a second context is worth its cost only when the maintainer asks for one, or when the work genuinely cannot be done here.

**Two full rounds of review stand between finished work and every commit.** Not two readings of the risky part: each round covers the whole surface that commit touches, including the tests and the documents, and the second takes an angle the first did not. What one round read for correctness the next reads for what the change costs the reader, for the state it leaves on the device, for the regression that now passes for a different reason, or for whatever that change is actually about; how many angles exist is a property of the change rather than of a fixed list, and two is the floor rather than the target. Inside a round, read a file or an area to its end and collect every finding in it before fixing anything. Stopping at the first one re-reads that file with the fix already in it, which is how the finding underneath goes unseen, and this project's own record is that the round after a fix is the one that finds the defect the fix introduced.

Where the work follows a plan, go back to the part of it that covers each logical block before implementing that block. A long task is what this is about: what the plan said drifts out of a session as the hours and the tool calls accumulate, and a block built against a summary made several blocks ago is built against the wrong thing. Where the plan is a document, that means reading its section from disk again rather than recalling it, and the section outranks what this session remembers of it; where it is an agreement in the conversation and nothing else, it means saying what this block is for before building it, so the maintainer can correct a drifted one while it is still cheap. This is a plan the work follows, and not the plan-shaped narrative this section refuses for a one-step operation.

Say what is happening while it happens, on every run and whether or not there is a plan behind it. The maintainer is steering a run they pay for, and a run that goes quiet for fifty tool calls cannot be steered until it is finished and wrong. What is owed is the milestones rather than a line per tool call: what is being worked on now, when it is finished, which review round is running, and anything that changes what the rest of the run will do, said at the moment it is decided rather than in the report at the end. Where there is a plan, the section being returned to is one of those milestones.
...
- `FAILED`: the run names the cases that failed and prints the runner's own account of the first one, so fix the cause from what it printed rather than running the selection again to read it. `build/qa/<run-id>/qa-failures.json` carries the same, and `--name` narrows a `--pytest` or `--vitest` selection to one case while fixing it.
...
- One run at a time, waited for where it was started. Two runs on one tree share `build/qa/reuse.json` with no lock, so each overwrites what the other proved, and the component suite's waits time out on a machine that is also running a gate. Bound the run and wait no longer than the bound: on the Steam Machine `timeout 150`, because a `release` with nothing reused measured 124.0 and 124.1 seconds of stage time there over two runs on 2026-09-23 (84 seconds on 2026-09-14, in `docs/FIELD_NOTES.md`; `backend-full` and `frontend-component` have grown since, to about 48 and 66 seconds). That figure is this machine's. A Steam Deck, a laptop, a container or a shared runner is slower, so take the first full run's own stage timings on the host you are on and bound the next one just above them; a bound below what a host honestly costs fails a healthy tree and invites the gate to be blamed for the change. A run still going past a bound sized that way is a hang in what is being validated rather than a slow gate, and the ordinary causes are a timer the new code left running and a loop whose exit cannot be reached. The bound is what turns that into a failed run you can read; a foreground wait offers no moment in which to notice.
...
Wait for CI on the head you pushed, in the foreground, and report what it said. One documented command does that wait and proves the run belongs to the exact head; it is in `docs/DEVELOPMENT.md` beside the inspection commands, and assembling one out of a run listing and a watcher spends calls on a question already answered there. A push is not the end of the work: until green exact-head CI is read the head is pushed, not ready, and a red run is the finding. Documentation-only changes need repository QA rather than CI.
...
| What did the plugin's own updater do while the plugin was being replaced | `scripts/target_plugin_log.py --updater`; the install happens in a detached process that outlives this backend, so none of it is in the backend log |
...
| What is this plugin's panel showing right now, and can a row be reached | `scripts/target_panel_read.py`; it reads each row, its controls and their state out of the page as text. `--open`/`--reopen` opens this plugin's own panel through Steam's and Decky's own calls, and `--press <name>` activates any control this plugin draws (`--press 'Advanced…'`, `--press Manage`, repeated to go deeper), including a switch by the name of the control; `--choose <option>` picks one option of a dropdown through Steam's own menu, `--describe <data-testid>` prints one row's whole text and every control in it, and `--wait-for <data-testid>:<text>` waits for a screen that is still doing something. So reaching a screen and looking at it needs nobody at the device. It presses the controls that write too, including **Use**, **Apply** and **Delete**: every press is reported, and naming them to the maintainer is required, never optional. Prefer it to a capture for anything a layout question turns on |
...
| What does a table switch on by itself, and which of those may be switched off | `scripts/ct_scan_survey.py --table <table> --flags`; it reads the records' own answers through the production inspector and names every flag the hook reader refused, which is where a count about a table's flags comes from rather than from a line written by hand |
| What does a table scan for, and does this game's program hold it | `scripts/ct_scan_survey.py`; `--tables` over a directory reports what the production parser read out of real tables and what it refused, with the reason and an example of each, and `--table` and `--executable` runs the check the panel runs and prints which patterns are present, which are missing and what it cost. `--tables` with `--repair-coverage` answers how much of a corpus the repair can produce a proven table for, and how much of that a user can be offered. Read only: no table is stored and no game is started |
...
- A commit is not a default end state, and none is made without the maintainer asking for one. A request for the work itself is not a request for a commit: asking for a diagnosis, for a durable conclusion to be written down, for a document to be updated or for a defect to be fixed asks for the change, and the change is finished when it is in the working tree and its QA has passed. The report names the exact paths touched and the selection that covered them, and the maintainer, who is reading the same tree, decides what becomes history and under what message. This is about whose history it is rather than about risk: a commit nobody asked for writes a message and a boundary into a public log on that person's behalf. Work spanning several logical blocks is the exception, and it is an exception to the asking rather than to anything else: each block is committed as it is finished and its two review rounds are done, without a request of its own, because a single commit for all of it is a message about everything and a boundary about nothing, and because a block nobody can name is a block nobody can revert. Every other condition holds, including the two rounds, the staging of named paths and the QA that covers what was changed; publication still waits to be asked for; and a change small enough to be one commit still waits in the working tree. A commit of its own is not a gate of its own: the selection that covers a block is what that block is proved with, and the full gate is spent once over the batch those commits add up to, exactly as it is for any other batch. Do not commit operational installs or unchanged evidence. Push, create/move tags, publish releases, or update PRs only when the maintainer explicitly requests publication; that request carries the commits the push needs, because an unpushed tree has nothing to publish. Publication covers the finished batch at that point; later edits wait in the working tree until the maintainer asks again.
- A round of static review is the standing exception to waiting for a request to publish. While the maintainer is bringing review findings here, push the finished batch as soon as it is validated rather than asking: the reviewer runs elsewhere and reads the pushed branch, so an unpushed fix is invisible to the round that would confirm it. CI is still awaited and reported, and the commits that push needs are made as part of it. Work outside such a round waits in the working tree.
...
For Game Mode UI evidence use `target_screenshot.py`, never a photograph: `--region qam` or `center` to crop, `full` only for context, a descriptive `--prefix`. Prefer `target_panel_read.py` to a capture for anything a layout question turns on, since it reports each row, its controls and their state for a fraction of what a frame costs to read. Reach the screen yourself rather than asking for it: `target_panel_read.py --reopen --press 'Advanced…'` opens this plugin's own screens through Steam's and Decky's own routes and the handlers this plugin wrote, repeated to go deeper. It presses the controls that authorize, download, write or destroy as well, because a screen behind such a press is still a screen that has to be looked at - and in return **every press it makes is named to the maintainer**, in the run's own output and again in whatever is reported from it. A press nobody was told about is the defect; the press itself is not. Say so before driving a device, prefer a read where a read answers the question, and do not press what the task did not ask for. A name that is on more than one control presses nothing until it is written `<data-testid>:<name>`. A running game and Steam's own interface are what still needs the maintainer. Installing a package replaces Steam's webhelper and closes the Decky UI, so a frame taken straight afterwards shows only what the session fell back to: reopen the panel and press back to the screen before capturing. Captures are temporary evidence, and only durable conclusions are kept.
...
**A panel that is dead after an install is never recovered with a `kill`.** `pkill -f steamwebhelper` matches the whole Steam client and takes Game Mode down with it, and `plugin_loader` is root-owned, so nothing an agent can run stops either one. The reload is the other half of this: a timed-out reload RPC says the answer did not arrive, never that the reload did not happen, and one has fired three minutes after its own timeout. Wait that out and ask `target_plugin_install.py authority` again before concluding anything, then read `target_plugin_log.py --frontend` for whether the panel mounted. Where it is still absent, say so and hand the restart to the maintainer rather than reaching for a signal: `sudo systemctl restart plugin_loader` for the loader, and a restart of Steam itself for a webhelper Steam keeps replacing. `docs/FIELD_NOTES.md` carries what that replacement loop is and why neither side leaves it.
