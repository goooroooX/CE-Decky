...
Where the maintainer is working from a plan document, which is a file this repository does not track and is pointed at rather than found, read that plan's own section from disk again before implementing each logical block. Not once at the start: a block is built against what its section says rather than against a summary of it made several blocks ago, and the section outranks what this session remembers about it. This is the plan a change is organised by, and not the narrative the paragraph below refuses for a one-step operation.

Say what is happening while it happens. The maintainer is steering a run they pay for, and a run that goes quiet for fifty tool calls cannot be steered until it is finished and wrong. Report each plan section as it is re-read, which block is being built now, when one is finished, and which review round is running; and say anything that changes what the rest of the run will do at the moment it is decided rather than in the report at the end. The milestones are what is owed, not a line per tool call.
