...
The check reads the game's own program, and the other files the table names where they sit beside it, so a table that finds its code in a game's engine library is checked there too. Code is only ever taken out of a copy when the table looks for it in the game's own program, which is the one file CE Decky knows the game runs. A table can also tell Cheat Engine to look anywhere in the running game, or in another file the game loads; then not finding the code on disk proves nothing, so review tells you what was not found and the copy leaves those cheats alone. It runs only where CE Decky already knows which program the game runs, which means the game is running now or a cheat from it worked here before. For a game this device has never started, review says nothing about any of this rather than guessing.
