"""Strict native extractor for the reviewed Cheat Engine 7.7 installer.

This production module is deliberately independent of tools/ce77_extract.py.
The latter stays as a standalone investigation tool; this module adapts its
verified CE 7.7 format knowledge to CE Decky's managed transaction boundary.
...
class CE77ExtractionError(ValueError):
    """The pinned CE 7.7 artifact was validly read but could not be extracted."""


def _installer_identity(path: Path, expected_size: int) -> tuple[int, int, int, int]:
    try:
        info = path.stat(follow_symlinks=False)
    except OSError as exc:
        raise CE77ExtractionError("reviewed installer is unavailable") from exc
    if path.is_symlink() or not stat.S_ISREG(info.st_mode):
        raise CE77ExtractionError("reviewed installer must be a regular non-symlink file")
    if info.st_size != expected_size:
        raise CE77ExtractionError("reviewed installer size changed before native extraction")
...
def extract_reviewed_installer(
    installer: Path,
    output: Path,
    *,
    expected_size: int,
    ...
    """Extract only the reviewed CE 7.7 artifact without executing it.

    The managed-install manifest remains the outer artifact authority.  This
    narrow format reader additionally refuses a manifest that tries to route a
    different artifact through CE 7.7-specific parsing.  Output must be a new
    plugin-owned staging directory; promotion is owned by ``managed_ce``.
    """
    if expected_size != EXPECTED_SIZE or expected_sha256 != EXPECTED_SHA256:
        raise CE77ExtractionError(
            "the reviewed artifact does not match the CE 7.7 native extractor"
        )
    if output.exists() or output.is_symlink():
        raise CE77ExtractionError("native extraction staging directory already exists")
    before = _installer_identity(installer, expected_size)
    try:
        result = run(installer, output, False, raise_errors=True)
    except CE77ExtractionError:
        raise
    if result != 0:  # Defensive: raise_errors always makes failures exceptional.
        raise CE77ExtractionError("native CE 7.7 extraction did not complete")
    if _installer_identity(installer, expected_size) != before:
        raise CE77ExtractionError("reviewed installer changed during native extraction")


def run(
    installer: Path,
    output: Path,
    allow_other_hash: bool,
    *,
    ...
    try:
        ...
        if raise_errors:
            raise CE77ExtractionError(
                f"native CE 7.7 extraction failed at stage '{stage}': {exc}"
            ) from exc
