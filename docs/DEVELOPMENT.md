...
It serialises every surface this plugin is drawing: each row by its `data-testid`, the text actually rendered, the controls beside it with their disabled and focused state, how many of its lines are clipped, whether the panel is drawing its mascot at all, and with `--metrics` the height of each row and of the surface. That is a dozen lines where a capture is a megabyte, and it answers the questions a layout change is about. `target_screenshot.py` stays for what a picture is actually for: colour, overlap, and anything about the frame itself.
