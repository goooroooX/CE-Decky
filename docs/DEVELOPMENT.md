...
It serialises every surface this plugin is drawing: each row by its `data-testid`, the text actually rendered, the controls beside it with their disabled and focused state, how many of its lines are clipped, whether the panel is drawing its mascot at all, and with `--metrics` the height of each row and of the surface. Buttons that sit in no row, such as Steam's full-width **Load table & start CE** and **Configure cheats**, which carry no test id, are listed under an `untagged-controls` row of the surface they are on, which `--wait-for` and a scoped `--press untagged-controls:<name>` take like any other row. That is a dozen lines where a capture is a megabyte, and it answers the questions a layout change is about. `target_screenshot.py` stays for what a picture is actually for: colour, overlap, and anything about the frame itself.

`--open` opens the quick access panel on this plugin's own page first, through Steam's own `MenuStore.OpenQuickAccessMenu` reached the way `@decky/ui` reaches it, and Decky's own `deckyState.setActivePlugin`. `--reopen` closes it through Steam's own `CloseSideMenus` and opens it again, and it is the one to use after changing anything the backend holds: the panel re-reads the backend when it comes back into view, so a panel that never went away has no reason to have noticed, and reading it then reports the state before the change as though it were the state after it. Both are API calls rather than injected input. Past that first screen, `--press <name>` activates one of this plugin's own controls by the name on it and can be repeated to go deeper, which is how a modal is reached without anybody at the device. **Any of this plugin's controls, and every press is reported.** This is a testing tool, and the screen behind a press that writes is still a screen somebody has to look at, so **Use**, **Apply** and **Delete** are reachable; what that costs is a rule rather than a refusal. Each press is named on its own line as it lands, with the surface it was found on, a run that pressed anything ends with the list of what it pressed in order, and `--json` carries the same list. Repeat that list when reporting what the run found: a press the operator was never told about is the defect the rule exists to prevent, not the press itself. Say it before driving a device that is not yours to write to, and prefer a read to a press where a read answers the question. Two limits remain, and neither is about which name is allowed. A name that is on more than one control presses nothing and reports where each of them is, because **Search** opens the search screen from the panel and spends a provider request inside it, **Cancel** leaves a dialog and stops a running Cheat Engine installation on the panel, and **Use** is on every row of a list; write `<data-testid>:<name>` to name the one row instead, which is how `--press table-row:Search` reaches the search screen and `--press imported-table-33a60e6b:Use` reaches one row's own press. And a disabled control is reported as disabled rather than activated. A running game and Steam's own interface are what still needs the maintainer.

Four more things a device session needs, each of which was a dead end before it existed. A switch is pressed by the name of the control rather than by a label, because Steam draws one as a `div` with no name at all: this plugin marks its own with `data-switch`, the reader reports each one with what it is showing, and `--press <row>:<name>` flips it - without that, a reader could report every row of a screen and reach none of the switches, which is most of what this product is. `--choose <option>` picks one option of a dropdown by the text a reader sees, which is Steam's own context menu rather than anything this plugin drew, so it opens the control, waits for the menu, matches the option, and where it cannot, closes the menu again and prints the options that were there; it is a write and is reported exactly as a press is. `--describe <data-testid>` prints one row's whole text, up to four thousand characters, and every control inside it with its tag, role, classes and `data-` marks, which is both how a finding block several sentences long is read in full and how a control this reader cannot yet name is identified. And `--wait-for <data-testid>:<text>` waits, bounded by `--wait-seconds`, until that row says that: a stop, an install or an activation is a screen that is still doing something, and polling it by hand is how a walk turns into a busy loop. A press returns once the click is dispatched, not once what it asked the backend for has happened: **Clear** was reported pressed seconds before the backend cleared the hold, so a probe read straight afterwards still showed it. Read the outcome through `--wait-for`, or after the backend log says it happened.
...
| `target_ce_launch_probe.py` | **To prove Cheat Engine actually starts, without a game, a table or a person at the device.** It is the agent-verifiable half of an attached launch; the other half needs the maintainer. Exercise the production CE/Proton launch and bridge path in its bounded self-test mode. `--table <path>` puts an exact `.CT` in front of that Cheat Engine instead of the synthetic one, and the report's `bridge` block then answers `table_load_state`, `table_load_error`, `table_load_route` and `address_list_count` for it: the bridge loads the session table in its bootstrap, before the attach and without depending on a target process, so this says whether this exact Cheat Engine opens this exact table with no game, no profile, no consent and no Steam state. Measured on the Steam Machine at 4.1 s per table, three runs. It does not inspect the bytes first, so it can be pointed at a file the table store would refuse, and the load runs the table's own Lua and Auto Assembler exactly as the production bridge does, so it is execution rather than inspection. |
...
| `table_ui_probe.py` | **Before assuming a real table is usable in Game Mode.** A table with thousands of records can put its controls out of reach of a controller, and that is a property of the table rather than of the code. Report whether a real table's controls stay reachable in the QAM control browser, through the production CT inspector and without executing anything the table carries. It also reports how many of a table's records the picker draws as a plain switch rather than a list, the keys each of those writes for off and on, and the two-entry lists whose labels the switch vocabulary could not place, which is what a change to that vocabulary is judged on. `--summary` aggregates all of that over every table given instead of reporting one table at a time, which is how a figure over a corpus is taken: `python3 scripts/table_ui_probe.py --summary $(find build/corpus/store/sha256 -name '*.CT')`. |
| `ct_scan_survey.py` | **When what a table actually scans for, or whether a game's program still holds it, has to come from real files** rather than from what the Auto Assembler manual permits. `--tables` walks a directory of tables through the production parser and reports how many scans it read, how many scripts declare none, the shortest, median and longest pattern, how many have no literal first byte, which of Cheat Engine's three scanners each is looked for with, how many symbols stand for more than one pattern and in how many tables, and every line it refused with the reason and an example. `--table` and `--executable` runs the check the panel runs and prints which patterns the program holds, which it does not, which could not be checked and why, which symbols the table uses for more than one pattern, and what each run cost; `--repeat <n>` gives the spread. `--table <t> --repair <scan>` produces the repair that drops that scan, runs both of its proofs over the result and reports what it costs - how many blocks and lines went, how many bytes, and which cheats lost the address they were reached by - without storing anything; repeat `--repair` for several scans. `--tables <dir> --repair-coverage` does that for every scan of every table in a directory and counts what survived both proofs: how many scans are repairable in total and by directive, how many tables have at least one and how many are repairable entirely, and how many of those a repair can actually be offered for, which is the subset whose directive names the file the pattern is looked for in. It is the answer to how much of a corpus this feature reaches, and it runs the transform once per scan, so it is the slowest mode here. `--table <t> --flags` answers the other question a table asks of this project: how many of its records its own scripts switch on by themselves, how many of those the hook reader established as safe to write off, and the name of every one it did not, which is the list of cheats CE Decky leaves running when it starts a script for something else. It reads the record's own answer through the production inspector, so it is what the product acts on rather than a second opinion about it; `--tables <dir> --flags` gives the same counts over a corpus, with how many tables switch anything on at all and how many have no flag the reader had to refuse. A count stated anywhere about a table's flags comes from here, because a number produced by hand is one nobody can reproduce. Read-only: no table is stored, no game is started, and the program is opened through the same bounded reader the product uses. It has no platform requirement and runs anywhere, so a timing it prints is that machine's and is reported with the machine and the file's size beside it. `python3 scripts/ct_scan_survey.py --tables build/corpus/store/sha256` |
| `pe_version_survey.py` | **When a bound in the version reader has to be chosen from what executables actually do** rather than from what the PE format permits. Walk one or more directories of `.exe` through the production reader and report, per file, the declared version, the size of the whole resource section, how far into it the resource directories reached and how many spans the read cost, with the largest of each in the summary. Read-only and executes nothing: headers and resources are parsed, never mapped or run. It has no platform requirement and runs anywhere, so the report names the host it ran on, and roots are given with `--root` rather than guessed. `python3 scripts/pe_version_survey.py --root <games directory>` |
...
```bash
python3 scripts/target_plugin_log.py --lines 200 --grep 'fearless|session'
python3 scripts/target_plugin_log.py --plugin-root '<exact-plugin-root>' --lines 200
```

The first form is the ordinary one: the root comes from the install this
project's own installer recorded, which is the same answer
`target_plugin_install.py authority` prints. Name it only where this device
holds an installation that installer did not make.
...
| What a self-update did | `target_plugin_log.py --updater --json` |
