...
`--open` opens the quick access panel on this plugin's own page first, through Steam's own `MenuStore.OpenQuickAccessMenu` reached the way `@decky/ui` reaches it, and Decky's own `deckyState.setActivePlugin`. `--reopen` closes it through Steam's own `CloseSideMenus` and opens it again, and it is the one to use after changing anything the backend holds: the panel re-reads the backend when it comes back into view, so a panel that never went away has no reason to have noticed, and reading it then reports the state before the change as though it were the state after it. Both are API calls rather than injected input. Past that first screen, `--press <name>` activates one of this plugin's own controls by the name on it and can be repeated to go deeper, which is how a modal is reached without anybody at the device. It presses only the controls that open a screen - the list is `NAVIGATION_PRESSES` in the helper itself, and it names them in its refusal - so **Use this table**, **Delete these** and **Apply** are not reachable through it, and a disabled control is reported as disabled rather than activated. A name is matched against every surface this plugin draws, so a name that means one thing here and another there is not on that list: **Search** opens the search screen from the panel and spends a provider request inside it, **Cancel** leaves a dialog and stops a running Cheat Engine installation on the panel. Those are written `<data-testid>:<name>` and looked for only inside that one row, which is how `--press table-row:Search` reaches the search screen and nothing else. A screen behind one of those, a running game and Steam's own interface are what still needs the maintainer.
